...
public class BukkitConfigurationNodeImpl implements ConfigurationNode {

    private final FileConfiguration configuration;
    private final String path;

    BukkitConfigurationNodeImpl(FileConfiguration configuration, String path) {
    ...
    public ConfigurationNode getNode(String path) {
        if (configuration.contains(this.path + "." + path)
            ...
            return new BukkitConfigurationNodeImpl(configuration, this.path + "." + path);
    ...
    public List<ConfigurationNode> getChildNodes() {
        ...
        return section.getKeys(false).stream()
                .map(key -> new BukkitConfigurationNodeImpl(configuration, path + "." + key)).collect(Collectors.toList());
    }
